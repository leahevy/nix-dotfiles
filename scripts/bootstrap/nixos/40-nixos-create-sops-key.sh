...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  HOME="$(nix eval --impure --json --override-input config "path:$CONFIG_DIR" --override-input profile "path:$PROFILE_PATH" ".#nixosConfigurations.$FULL_PROFILE_NAME.config.users.users.$USERNAME.home")"
  ...
  if [[ -f "/mnt/etc/sops/age/keys.txt" && -f "$USER_SOPS_DIR/keys.txt" ]]; then
    ...
    if [[ ! -f "$USER_SOPS_DIR/keys.txt" ]]; then
      ...
      USER_UID="$(nix eval --impure --json --override-input config "path:$CONFIG_DIR" --override-input profile "path:$PROFILE_PATH" ".#nixosConfigurations.$FULL_PROFILE_NAME.config.users.users.$USERNAME.uid")"
      GROUP_NAME="$(nix eval --impure --json --override-input config "path:$CONFIG_DIR" --override-input profile "path:$PROFILE_PATH" ".#nixosConfigurations.$FULL_PROFILE_NAME.config.users.users.$USERNAME.group")"
      ...
      if [[ -z "$USER_UID" || "$USER_UID" == "null" || -z "$GROUP_NAME" || "$GROUP_NAME" == "null" ]]; then
        ...
        USER_GID="$(nix eval --impure --json --override-input config "path:$CONFIG_DIR" --override-input profile "path:$PROFILE_PATH" ".#nixosConfigurations.$FULL_PROFILE_NAME.config.users.groups.$GROUP_NAME.gid")"
