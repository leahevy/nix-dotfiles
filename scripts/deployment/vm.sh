...
while [[ $# -gt 0 ]]; do
	...
	--select)
		[[ $# -lt 2 ]] && {
			print_error "--select requires a version name"
			exit 1
		}
		SELECT_VERSION="$2"
		shift 2
...
if [[ "${WILL_RUN_VM}" == "true" ]]; then
	...
	if [[ -n "${AGE_FILE}" ]]; then
		[[ -f "${AGE_FILE}" ]] || {
			print_error "Age key file does not exist: ${AGE_FILE}"
			exit 1
		}
	elif [[ -n "${AGE_SYSTEM_FILE}" ]]; then
		[[ -f "${AGE_SYSTEM_FILE}" ]] || {
			print_error "System age key file does not exist: ${AGE_SYSTEM_FILE}"
			exit 1
		}
		if [[ "${NO_USER_AGE}" != "true" ]]; then
			[[ -f "${AGE_USER_FILE}" ]] || {
				print_error "User age key file does not exist: ${AGE_USER_FILE}"
				exit 1
			}
		fi
	else
