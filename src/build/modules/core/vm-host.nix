  ...
  module =
    ...
    in
    {
      ...
      home =
        { config, ... }:
        let
          dataPath = self.host.settings.system.vmsDataPath;
          defaultPersistDir =
            if lib.hasPrefix "~/" defaultDataPath then
              lib.removePrefix "~/" defaultDataPath
            else
              defaultDataPath;
          finalResolvedDataPath = if dataPath == null then defaultDataPath else dataPath;
          vmDataDir =
            if finalResolvedDataPath == "~" then
              "$HOME"
            else if lib.hasPrefix "~/" finalResolvedDataPath then
              "$HOME/${lib.removePrefix "~/" finalResolvedDataPath}"
            else
        ...
        in
        {
          ...
          home.persistence."${self.persist}".directories =
            lib.optionals (finalResolvedDataPath == defaultDataPath)
              [
                defaultPersistDir
              ];
        };
