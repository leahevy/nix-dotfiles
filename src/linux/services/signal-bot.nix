  ...
  texts = {
    en = {
      ...
      memInstruction = ''
        ...
        4. Maintain a section titled People. This section captures who each person is, not what they have been doing lately; descriptors must be stable facts that would read the same regardless of which period is being written. Every person gets their own individual entry no matter how little is known about them; never group multiple people into one line or bullet. For each known person write their name followed by compact key: value descriptors covering only persistent identity: role or relationship, recurring personality traits, lasting preferences. Use short phrases or single words per property, separated by commas or semicolons. Never let time-bound information enter this section: schedules, shifts, appointments, plans, activities, or opinions from any specific period do not belong here, even when they recur. Everything a person did, said, or has planned goes in the chat sections or the Ongoing section, not here. Note which chat you first learned the identity facts in. Never remove a person from this list once they appear; if someone has been absent from recent transcripts you may shorten their entry to save space, but never omit them entirely. People who live in the household must always remain with at least their name and role, no matter how long they have been absent from the transcripts.
    ...
    de = {
      ...
      memInstruction = ''
        ...
        4. Pflege einen Abschnitt mit dem Titel Personen. Dieser Abschnitt erfasst, wer eine Person ist, nicht was sie zuletzt getan hat; die Beschreibungen müssen stabile Fakten sein, die unabhängig davon, welche Periode gerade geschrieben wird, gleich lauten würden. Jede Person bekommt einen eigenen Eintrag, egal wie wenig über sie bekannt ist; fasse niemals mehrere Personen in einer Zeile oder einem Listenpunkt zusammen. Schreibe für jede bekannte Person ihren Namen, gefolgt von kompakten Schlüssel: Wert-Beschreibungen, die nur dauerhaft gültige Identitätsmerkmale erfassen: Rolle oder Beziehung, wiederkehrende Charaktereigenschaften, bleibende Vorlieben. Verwende kurze Phrasen oder einzelne Wörter pro Eigenschaft, getrennt durch Komma oder Semikolon. Lass niemals zeitgebundene Informationen in diesen Abschnitt einfließen: Arbeitszeiten, Schichten, Termine, Pläne, Aktivitäten oder Meinungen aus einer bestimmten Periode gehören auch dann nicht hierher, wenn sie sich wiederholen. Alles, was eine Person getan hat, gesagt hat oder plant, gehört in die Chat-Abschnitte oder in den Abschnitt Laufendes, nicht hierher. Gib an, in welchem Chat du die Identitätsfakten erstmals erfahren hast. Entferne niemals eine Person aus dieser Liste, sobald sie darin erscheint; ist jemand in den letzten Protokollen nicht vorgekommen, darf der Eintrag zur Platzersparnis gekürzt werden, aber die Person selbst darf niemals weggelassen werden. Personen, die im Haushalt leben, müssen immer mit mindestens Name und Rolle verbleiben, unabhängig davon, wie lange sie nicht in den Protokollen erschienen sind.
