    ...
    ifEnabled.linux.server.healthchecks = {
      enabled =
        ...
        lib.mkIf config.nx.linux.server.auth.enableOAuthProxy {
          ...
          nx.linux.server.healthchecks.regularHealthChecks."+52 - OAuth2-Proxy ready" = ''
            _code=$(${pkgs.curl}/bin/curl -s -o /dev/null -w "%{http_code}" --max-time 10 \
              "http://127.0.0.1:4180/ready" 2>/dev/null || true)
            printf 'http://127.0.0.1:4180/ready - HTTP %s\n' "$_code" >&3
            [[ "$_code" =~ ^2 ]]
          '';
        };
    };
