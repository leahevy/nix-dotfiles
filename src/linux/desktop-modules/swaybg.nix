      ...
      let
        ...
        seasonWallpaperFiles = builtins.listToAttrs (
          map (
            entry:
            let
              w = if isWidescreen then entry.widescreen else entry.normal;
            in
            {
              name = ".config/swaybg/wallpapers/${w.name}";
              value = {
                source = w.path;
              };
            }
      ...
      lib.mkMerge [
        {
          ...
          home.file.".local/bin/swaybg-switch-wallpaper" = {
            ...
            text = ''
              ...
              STYLIX_BASENAME=""
              for f in "$WALLPAPERS_DIR"/stylix.*; do
                if [[ -f "$f" || -L "$f" ]]; then
                  STYLIX_BASENAME="$(${pkgs.coreutils}/bin/basename "$f")"
                  break
                fi
              done

              FUZZEL_INPUT=""
              if [[ -n "$STYLIX_BASENAME" ]]; then
                if [[ "$CURRENT_WALLPAPER" == "$STYLIX_BASENAME" ]]; then
                  FUZZEL_INPUT="● Stylix Wallpaper"
                else
                  FUZZEL_INPUT="○ Stylix Wallpaper"
                fi
              fi
              ...
              if [[ -n "$SELECTED" ]]; then
                ...
                if [[ "$SELECTED_BASENAME" == "Stylix Wallpaper" ]]; then
          ...
          home.file.".local/bin/scripts/swaybg-update-wallpaper" = {
            ...
            text = ''
              ...
              RANDOMIZE_DEFAULT=${if self.settings.randomize then "1" else "0"}
              RANDOMIZE=0
              if [[ "''${1:-}" == "--randomize" ]]; then
                RANDOMIZE=1
              else
                STYLIX_FILE=$(${pkgs.findutils}/bin/find "$WALLPAPERS_DIR" -maxdepth 1 \( -type f -o -type l \) -name "stylix.*" 2>/dev/null | head -n 1)
                if [[ ! -f "$STATE_FILE" ]]; then
                  if [[ -n "$STYLIX_FILE" && "$RANDOMIZE_DEFAULT" != "1" ]]; then
                    WALLPAPER="$STYLIX_FILE"
                  else
                    RANDOMIZE=1
                  fi
                else
        ...
        {
          home.file = seasonWallpaperFiles;
        }
      ];
  };
}
