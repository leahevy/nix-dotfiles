  ...
  configuration =
    context@{ config, options, ... }:
    {
      home.file.".config/nvim-init/50-highlight-dead-chars.lua".text = ''
        ...
        ${lib.optionalString self.settings.eolFill.enabled ''
          ...
          local function add_eol_fill()
            if is_excluded_filetype() or vim.bo.buftype ~= "" then
              return
            end
