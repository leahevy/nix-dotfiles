    ...
    home =
      ...
      let
            ...
            map (
              ...
              map (
                ...
                let
                  ...
                  localPath = if builtins.isString shareConfig then shareConfig else shareConfig.path;
                in
                "${self.user.home}/${localPath}"
              ) (builtins.attrNames (device.shares or { }))
            ) self.settings.devices
          )
        );
        ...
        parentDir =
          ...
          in
          if parentParts == [ "" ] || parent == "" then "/" else parent;

        parentDirs = lib.unique (
          lib.filter (p: p != "/" && p != self.user.home) (map parentDir allSharePaths)
        );

        preCheckScript = pkgs.writeShellScript "syncthing-check-share-parents" (
          ''
            exit_code=0
          ''
          + lib.concatMapStringsSep "\n" (dir: ''
            if [ ! -d "${dir}" ]; then
              echo "ERROR: Parent directory '${dir}' does not exist for a configured Syncthing share"
              exit_code=1
            else
              echo "OK: Parent directory '${dir}' exists"
            fi
          '') parentDirs
          + ''

            if [ $exit_code -eq 0 ]; then
              echo "All Syncthing share parent directories verified successfully"
            fi
            exit $exit_code
          ''
        );
                          ...
                          if [[ "$should_notify" == "true" ]]; then
                              ...
                              case "$message" in
                                  *"NAT-PMP"*|*"UPnP"*)
                                      ...
                                      ;;
                                  *"WRN Failed to exchange Hello messages"*)
                                      # Ignore transient TLS/protocol negotiation failures
                                      ;;
        ...
        syncthingFolderBasedMonitor = pkgs.writeShellScript "syncthing-folder-based-monitor" ''
          set -euo pipefail
      ...
      in
      {
        ...
        systemd.user.services.syncthing = lib.mkMerge [
          ...
          (lib.mkIf (parentDirs != [ ]) {
            Unit = {
              After = lib.mkAfter [ "syncthing-check-shares.service" ];
              Requires = lib.mkAfter [ "syncthing-check-shares.service" ];
            };
          })
        ];
        ...
        systemd.user.services.syncthing-check-shares = lib.mkIf (parentDirs != [ ]) {
          Unit = {
            Description = "Check Syncthing share parent directories exist";
          };
          Service = {
            Type = "oneshot";
            RemainAfterExit = true;
            ExecStart = "${preCheckScript}";
          };
        };
        ...
        systemd.user.services.syncthing-monitor-status = lib.mkIf self.settings.monitoringEnabled {
          ...
          Service = {
            ...
            Restart = "on-failure";
            RestartSec = "10";
            ExecStart = "${syncthingMonitor} monitor";
            Environment = [
              "PATH=${
                lib.makeBinPath [
                  pkgs.bash
        ...
        systemd.user.services.syncthing-monitor-logs = lib.mkIf self.settings.monitoringEnabled {
          ...
          Service = {
            ...
            Restart = "on-failure";
            RestartSec = "10";
            ExecStart = "${syncthingMonitor} logs";
            Environment = [
              "PATH=${
                lib.makeBinPath [
                  pkgs.bash
          ...
          lib.mkIf self.settings.folderBasedMonitoringEnabled
            {
              ...
              Service = {
                ...
                Restart = "on-failure";
                RestartSec = "30";
                ExecStart = "${syncthingFolderBasedMonitor}";
                Environment = [
                  "PATH=${
                    lib.makeBinPath [
                      pkgs.bash
