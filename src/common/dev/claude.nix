    ...
    enabled =
      ...
      let
        ...
        baseInstructions = {
          ...
          ++ [
            "$NX_AGENTS_PLANS_DIR is always set by the claude wrapper to the full repo-scoped plans directory (e.g. ~/.local/share/nx/agents/plans/-home-user--project); it already contains the slug and is the plans dir itself, not a parent. Use it directly: `mkdir -p $NX_AGENTS_PLANS_DIR/archive && mkdir -p $NX_AGENTS_PLANS_DIR/tmp && ls $NX_AGENTS_PLANS_DIR`. Never create archive or tmp outside this path. The Read tool does not expand environment variables; to read a plan file, first run Bash 'echo $NX_AGENTS_PLANS_DIR' to get the literal path, then pass '<that-path>/<filename>' to Read. Never construct the path manually by guessing the home directory prefix."
