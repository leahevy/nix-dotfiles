  ...
  module = {
    home =
      ...
      let
        ...
        cRed = "\\033[1;38;5;196m";
        cGreen = "\\033[1;38;5;82m";
        cWhite = "\\033[38;5;15m";
        cReset = "\\033[0m";

        validHostNames = lib.filter (h: h != "*") (lib.attrNames processedHosts);

        sshConnectScript =
          let
            validArray = lib.concatStringsSep " " (map lib.escapeShellArg validHostNames);
          in
          ''
            #!/usr/bin/env bash
            set -uo pipefail
            ...
            if [ $# -ne 1 ]; then
              printf '${cWhite}Usage: ssh-connect <hostname>${cReset}\n' >&2
              exit 1
            fi
            ...
            found=false
            for h in "''${VALID[@]}"; do
              if [ "$HOST" = "$h" ]; then found=true; break; fi
            done
            if [ "$found" != "true" ]; then
              printf '${cRed}ssh-connect: %s is not configured in ~/.ssh/config${cReset}\n' "$HOST" >&2
              exit 1
            fi
            ...
            while true; do
              ...
              printf '${cWhite}ssh-connect: connection to %s ended (exit %d). Reconnect? [${cGreen}y${cWhite}|${cRed}N${cWhite}]${cReset} ' "$HOST" "$code" >&2
              answer=""
              read -r -n 1 answer </dev/tty 2>/dev/null || true
              printf '\n' >&2
        ...
        sshConnectHostFiles = builtins.listToAttrs (
          builtins.filter (x: x != null) (
            lib.mapAttrsToList (
              profileName: hostCfg:
              let
                ...
                hasInitrd = (hostCfg.remote.initrdSSHHostPrivateKey or null) != null;
                normalPort = toString (hostCfg.remote.port or 22);
                initrdPort = toString (hostCfg.remote.initrdSSHExposedPort or 2233);
                username = hostCfg.mainUser.username or self.user.username;
                normalEntryName = if username == profileName then profileName else "${username}.${profileName}";
                initrdEntryName = "initrd.${profileName}";
              in
              if remoteAddress == null || remoteAddress == "" || hostname == currentHostname then
                null
              else
                {
                  name = "${defs.binDir}/ssh-connect-${profileName}";
                  value = {
                    executable = true;
                    text =
                      let
                        probeFunctions = ''
                          probe_normal() {
                            [ -n "$(${pkgs.openssh}/bin/ssh-keyscan -T 3 -p ${normalPort} ${remoteAddress} 2>/dev/null)" ]
                          }

                          probe_initrd() {
                            [ -n "$(${pkgs.openssh}/bin/ssh-keyscan -T 3 -p ${initrdPort} ${remoteAddress} 2>/dev/null)" ]
                          }
                          ...
                          connect_and_check() {
                            ...
                            printf '${cWhite}ssh-connect-${profileName}: connection ended, re-probing...${cReset}\n' >&2
                            sleep 3
                            return 1
                          }
                        '';
                      ...
                      if !hasInitrd then
                        ...
                        ''
                          ...
                          while true; do
                            if probe_normal; then
                              printf '${cGreen}ssh-connect-${profileName}: normal SSH available, connecting...${cReset}\n' >&2
                              connect_and_check ${lib.escapeShellArg normalEntryName} && break
      ...
      in
      {
        ...
        home.file = lib.mkMerge [
          ...
          (lib.mkIf isManagingMachine (
            {
              "${defs.binDir}/ssh-connect" = {
                executable = true;
                text = sshConnectScript;
              };
            }
            // sshConnectHostFiles
          ))
